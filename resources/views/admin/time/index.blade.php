        ...
        <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
            <ul class="layui-tab-title">
                <a href="javascript:void(0)">
                    <li class="layui-this">时间轴管理</li>
                </a>
                <a href="{{url('admin/time/create')}}">
                    <li class="">新增时间点</li>
                </a>
            </ul>
            ...
            <table class="layui-table">
                ...
                @foreach ($data as $index => $items)
                    <tr >
                        ...
                        <td>{{$items->content}}</td>
                        <td width="150">{{$items->created_at}}</td>
                        <td >
                        ...
                        <td width="180">
                            <a class="layui-btn layui-btn-xs" href="{{url('admin/time/edit/'.$items->id)}}">修改</a>
                            @if(is_null($items->deleted_at))
                                <a class="layui-btn layui-btn-xs layui-btn-normal J-baseAjaxTodo"
                                   href="{{url('admin/time/destroy/'.$items->id)}}" title="你确定要删除吗？">删除</a>
                            @else
                                <a class="layui-btn layui-btn-xs layui-btn-warm J-baseAjaxTodo"
                                   href="{{url('admin/time/restore/'.$items->id)}}" title="你确定要删除吗？">恢复</a>
                                <a class="layui-btn layui-btn-xs layui-btn-danger J-baseAjaxTodo"
                                   href="{{url('admin/time/forceDelete/'.$items->id)}}" title="你确定要删除吗？">彻底删除</a>
                            @endif
                        </td>
                    </tr>
                @endforeach
